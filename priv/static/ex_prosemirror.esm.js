...
var ReplaceStep = /* @__PURE__ */ function(Step3) {
  ...
  ReplaceStep2.prototype.merge = function merge3(other) {
    ...
    if (this.from + this.slice.size == other.from && !this.slice.openEnd && !other.slice.openStart) {
      var slice3 = this.slice.size + other.slice.size == 0 ? Slice.empty : new Slice(this.slice.content.append(other.slice.content), this.slice.openStart, other.slice.openEnd);
...
var AddMarkStep = /* @__PURE__ */ function(Step3) {
  ...
  AddMarkStep2.prototype.apply = function apply2(doc) {
    ...
    var slice3 = new Slice(mapFragment(oldSlice.content, function(node, parent2) {
...
var RemoveMarkStep = /* @__PURE__ */ function(Step3) {
  ...
  RemoveMarkStep2.prototype.apply = function apply2(doc) {
    ...
    var slice3 = new Slice(mapFragment(oldSlice.content, function(node) {
...
Fitter.prototype.fit = function fit() {
  ...
  var slice3 = new Slice(content, openStart, openEnd);
  if (moveInline > -1) {
  ...
  for (var j = slice3.openStart; j >= 0; j--) {
    ...
    for (var i$1 = 0; i$1 < targetDepths.length; i$1++) {
      ...
      if (parent.canReplaceWith(index, index, insert.type, insert.marks)) {
        return this.replace($from.before(targetDepth), expand2 ? $to.after(targetDepth) : to, new Slice(closeFragment(slice3.content, 0, slice3.openStart, openDepth), openDepth, slice3.openEnd));
      }
    }
  }
...
Transform.prototype.replaceRangeWith = function(from2, to, node) {
  ...
  return this.replaceRange(from2, to, new Slice(Fragment.from(node), 0, 0));
};
...
import { Slice as Slice2, Fragment as Fragment2 } from "prosemirror-model";
...
function joinBackward(state, dispatch, view) {
  ...
  if ($cursor.parent.content.size == 0 && (textblockAt(before, "end") || NodeSelection.isSelectable(before))) {
    var delStep = replaceStep(state.doc, $cursor.before(), $cursor.after(), Slice2.empty);
...
function deleteBarrier(state, $cut, dispatch) {
  ...
  if (canDelAfter && (conn = (match = before.contentMatchAt(before.childCount)).findWrapping(after.type)) && match.matchType(conn[0] || after.type).validEnd) {
    if (dispatch) {
      var end = $cut.pos + after.nodeSize, wrap = Fragment2.empty;
      for (var i = conn.length - 1; i >= 0; i--) {
        wrap = Fragment2.from(conn[i].create(null, wrap));
      }
      wrap = Fragment2.from(before.copy(wrap));
      var tr = state.tr.step(new ReplaceAroundStep($cut.pos - 1, end, $cut.pos, end, new Slice2(wrap, 1, 0), conn.length, true));
  ...
  if (canDelAfter && textblockAt(after, "start", true) && textblockAt(before, "end")) {
    ...
    if (at.canReplace(at.childCount, at.childCount, afterText.content)) {
      if (dispatch) {
        var end$1 = Fragment2.empty;
        for (var i$1 = wrap$1.length - 1; i$1 >= 0; i$1--) {
          end$1 = Fragment2.from(wrap$1[i$1].copy(end$1));
        }
        var tr$1 = state.tr.step(new ReplaceAroundStep($cut.pos - wrap$1.length, $cut.pos + after.nodeSize, $cut.pos + afterDepth, $cut.pos + after.nodeSize - afterDepth, new Slice2(end$1, wrap$1.length, 0), 0, true));
...
import { PluginKey, Plugin as Plugin2 } from "prosemirror-state";
...
function keymap(bindings) {
  return new Plugin4({ props: { handleKeyDown: keydownHandler(bindings) } });
}
...
export {
  ...
  baseKeymap,
  blocks,
  schema_default as createSchema,
...
//# sourceMappingURL=ex_prosemirror.esm.js.map
